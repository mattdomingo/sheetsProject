-- Pinterest Ads Keyword Report table creation query
-- Contains daily performance metrics at the keyword level
CREATE OR REPLACE TABLE `sheets_base_schema.pinterest_ads__keyword_report` (
  ...
  `ad_group_name` STRING OPTIONS(description = 'Name of the ad group.'),
  `ad_group_id` STRING OPTIONS(description = 'The ID of the related Ad group.'),
  `keyword_id` STRING OPTIONS(description = 'Unique identifier of the keyword.'),
  `match_type` STRING OPTIONS(description = 'Type of match the keyword is tied to. Either Exact or Broad.'),
  `parent_type` STRING OPTIONS(description = 'Identifier of what grain the parent type is. Ad group or campaign.'),
  `keyword_value` STRING OPTIONS(description = 'The text value that makes up the keyword.'),
...
OPTIONS (description = 'Daily performance metrics at the Pinterest Ads keyword level'); 