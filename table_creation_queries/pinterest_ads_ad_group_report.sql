-- Pinterest Ads Ad Group Report table creation query
-- Contains daily performance metrics at the ad group level
CREATE OR REPLACE TABLE `sheets_base_schema.pinterest_ads__ad_group_report` (
  ...
  `created_at` STRING OPTIONS(description = 'Ad group creation time.'),
  `start_time` STRING OPTIONS(description = 'Ad group start time.'),
  `end_time` STRING OPTIONS(description = 'Ad group end time.'),
...
OPTIONS (description = 'Daily performance metrics at the Pinterest Ads ad group level'); 