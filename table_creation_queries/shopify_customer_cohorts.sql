CREATE OR REPLACE TABLE `sheets_base_schema.shopify_customer_cohorts` (
  `date_month` DATE OPTIONS(description = 'The calendar month the customer stats relate to.'),
  `customer_id` INT64 OPTIONS(description = 'The ID of the related customer.'),
  `first_order_timestamp` TIMESTAMP OPTIONS(description = 'The timestamp of the customer\'s first order.'),
  `cohort_month` DATE OPTIONS(description = 'The month the cohort belongs to, i.e the first month the customer had an order.'),
  ...
  `customer_cohort_id` STRING OPTIONS(description = 'Unique key representing a customer cohort.')
)
OPTIONS (description = 'This table contains customer cohort data by month from Shopify.');