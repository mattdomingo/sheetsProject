-- Balance Sheet table creation query
-- This table contains account balance information for analysis
CREATE OR REPLACE TABLE `sheets_base_schema.qbo_balance_sheet` (
  ...
  `amount` BIGNUMERIC OPTIONS(description = 'The total ending period balance.'),
  `converted_amount` BIGNUMERIC OPTIONS(description = 'The total ending period balance, converted with exchange rates applied if available.'),
  `account_ordinal` INTEGER OPTIONS(description = 'Integer value to order the account within final financial statement reporting. The customer can also configure the ordinal.')
)
