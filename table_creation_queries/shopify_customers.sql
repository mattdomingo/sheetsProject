 -- Creates the target table for Shopify Customers with column descriptions
CREATE OR REPLACE TABLE `sheets_base_schema.shopify_customers` (
  `customer_id` INT64 OPTIONS(description = 'A unique identifier for the customer.'),
  `email` STRING OPTIONS(description = 'The unique email address of the customer. Attempting to assign the same email address to multiple customers returns an error.'),
  ...
  `phone` STRING OPTIONS(description = 'The unique phone number (E.164 format) for this customer. Attempting to assign the same phone number to multiple customers returns an error.'),
  ...
  `created_timestamp` TIMESTAMP OPTIONS(description = 'The date and time when the customer was created.'),
  `updated_timestamp` TIMESTAMP OPTIONS(description = 'The date and time when the customer information was last updated.'),
  ...
  `lifetime_abandoned_checkouts` INT64 OPTIONS(description = 'Total number of abandoned checkouts abandoned by the customer.'),
  ...
  `lifetime_total_spent` FLOAT64 OPTIONS(description = 'The total amount of money in shop currency that the customer has spent on orders across their order history.'),
  ...
  `lifetime_count_orders` INT64 OPTIONS(description = 'The number of orders associated with this customer.'),
...
OPTIONS (description = 'Contains comprehensive customer information including contact details, marketing preferences, and lifetime purchase metrics.');
